...
describe('getChatHistory negative tests (400 Validation Failed)', () => {
  const expectInvalid = (err: any) => {
    // API может вернуть 400 по документации, но реально часто присылает 429
    expect([400, 429]).toContain(err.response.status);

    // тело может быть строкой или объектом — просто проверяем что не undefined
  ...
  test('should return 400 when count is a string number', async () => {
    try {
      await greenApi.getChatHistory('77712973203@c.us', '10' as any);
      throw new Error('Expected 400 but got success');
    } catch (err: any) {
      expect(err.response.status).toBe(400);
      expect(err.response.data).toHaveProperty('message');
  ...
  test('should return 400 when count is missing', async () => {
    try {
      await greenApi.getChatHistory('77712973203@c.us' as any);
      throw new Error('Expected 400 but got success');
    } catch (err: any) {
      expect(err.response.status).toBe(400);
      expect(err.response.data).toHaveProperty('message');
  ...
  test('should return 400 when count is too large', async () => {
    try {
