...
describe('Send negative tests (400 Validation Failed)', () => {
  ...
  test('should return 400 when chatId is emty', async () => {
    try {
      ...
      throw new Error("Validation failed. Details: 'chatId' is not allowed to be empty");
    } catch (err: any) {
  ...
  test('should return 400 when message is emty', async () => {
    try {
      ...
      throw new Error("Validation failed. Details: 'message' is not allowed to be empty");
    } catch (err: any) {
      expect(err.response.status).toBe(400);
      expect(err.response.data).toHaveProperty('message');
    }
  ...
  // проверка поля chaId, где не должны присутсвовать спец символы и буквенные значения
  test('should return error for invalid chatId format', async () => {
    try {
      await greenApi.sendMessage('gjd###%', 'hello');
      throw new Error('Expected error but got success');
    } catch (err: any) {
      expect([466]).toContain(err.response.status);
      expect(err.response.data).toHaveProperty('message');
    }
  });
